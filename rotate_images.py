...
if os.path.isfile(args["images"]):
    ...
    images = [f for f in os.listdir(args["images"]) if os.path.isfile(os.path.join(args["images"], f)) and f[0] != "."]

for image in images:
    image_name = image.split(".")[0]
    image_extension = image.split(".")[1]
    image_full_path = os.path.join(args["images"], image)
    print(f"[INFO] Processing {image_full_path}")
    ...
    for angle in range(90, 271, 90):
        ...
        print(f"[INFO] Generated {output_image_path}")